            ...
            <div class="app-card-body p-4">
              <nav aria-label="breadcrumb">
                ...
                <div class="row gx-5 gy-3">
                    @can('create motorpools')
                        <div class="col-lg-4">
                            <div class="app-card mb-5">
                                <div class="app-card-body p-4">
                                    <form action="{{ route('motorpool.store') }}" method="POST">
                                        @csrf
                                        <div class="mb-3">
                    ...
                    @endcan
                    <div class="@can("create motorpools") col-lg-8 @else col-lg-12 @endif">
                        <div class="row">
                            <div class="col-lg-8">
                                <form>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @forelse ($motorpools as $motorpool)
                                    <tr>
                                        <td>
                                          @if($motorpool->is_active) 
                                        ...
                                        <td>{{ $motorpool->name }}</td> 
                                        <td>{{ $motorpool->address }}</td> 
                                        <td>{{ $motorpool->contact_number }}</td> 
                                        <td class="text-end">
                                            @can('update motorpools')
                                                <a href="{{ route('motorpool.edit', $motorpool->id) }}" class="btn-sm btn-primary text-white"><i class="fa fa-pencil"></i></a>
                                            @endcan
                                            @can('delete motorpools')
                                                <a href="#" onclick="if(confirm('Are you sure to delete?')){ document.getElementById('delete-{{ $motorpool->id }}').submit() }" class="btn-sm text-white btn-danger"><i class="fa fa-trash"></i></a>
                                                <form id="delete-{{ $motorpool->id }}" action="{{ route('motorpool.destroy', $motorpool->id) }}" method="POST">@csrf @method('DELETE')</form>
                                            @endcan
                                        </td>
                                    </tr>
                                @empty
