                ...
                <div class="row">
                    ...
                    <div class="col-6 text-end">
                        {{-- @can('create petty cash') --}}
                            <a href="{{ route('salary.create') }}" class="btn btn-primary text-white"><i class="fa fa-plus"></i> Add Salary</a>
                        {{-- @endcan --}}
                        @can('create petty cash')
                    ...
                    <tbody>
                        @forelse ($ledgers as $index => $ledger)
                            <tr>
                                <td width="150px">
                                    @if($index > 0)
                                        @if($ledger->petty_cash_id != $ledgers[$index - 1]->petty_cash_id)
                                            {{ $ledger->effectivity_date->format('M d, Y') }}
                                        @endif
                                    @else
                                        {{ $ledger->effectivity_date->format('M d, Y') }}
                                    @endif
                                ...
                                <td>
                                    @if($index > 0)
                                        @if($ledger->petty_cash_id != $ledgers[$index - 1]->petty_cash_id)
                                            {{ optional($ledger->pettyCash)->payee }}
                                        @endif
                                    @else
                                        {{ optional($ledger->pettyCash)->payee }}
                                    @endif
                                ...
                                <td>{{ $ledger->category == "labor" ? toPeso($ledger->amount) : ''}}</td>
                                <td>{{ $ledger->category == "material" ? toPeso($ledger->amount) : ''}}</td>
                                <td>{{ $ledger->category == "others" ? toPeso($ledger->amount) : ''}}</td>
                                <td>{{ toPeso($ledger->onhand_amount) }}</td>
                                <td>{{ optional($ledger->updated_at)->format('M d, Y h:iA') }}</td>
                                <td class="text-end">
                                    @if($index > 0)
                                        @if($ledger->petty_cash_id != $ledgers[$index - 1]->petty_cash_id)
                                            @can("print petty cash")
                                                <a href="{{ route('petty-cash.print', $ledger->pettyCash->id) }}" class="btn-sm text-white btn-primary    "><i class="fa fa-print"></i></a>
                                            @endcan
                                            {{-- @can("delete petty cash")
                                                ...
                                                <form id="delete-{{ $ledger->id }}" action="{{ route('ledger.destroy', $ledger->id) }}" method="POST">@csrf @method('DELETE')</form>
                                            @endcan --}}
                                        @endif
                                    @else
                                            @can("print petty cash")
