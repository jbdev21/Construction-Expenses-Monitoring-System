                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="type" class="form-control">
                                            @foreach($types as $type)
                                                <option @if(Str::lower($category->type) == Str::lower($type)) selected @endif value="{{ $type }}">{{  $type }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                  
                                    <button type="submit" class="btn btn-primary text-white mt-3"><i class="fa fa-save"></i> Save Changes</button>
                                    <a href="{{ route('category.index') }}" class="btn btn-secondary text-white mt-3"><i class="fa fa-ban"></i> Cancel</a>
                                </form>
                            </div>
                        </div>
                    </div><!--//col-->
